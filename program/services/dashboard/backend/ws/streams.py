    ...
    try:
        while True:
            try:
                ...
                await websocket.send_json({"type": "error", "data": {"message": "Data fetch failed"}})
        ...
        while True:
            try:
                ...
                await websocket.send_json({
                    ...
                    "data": {
                        "recent_signals": [
                            {k: v.isoformat() if hasattr(v, "isoformat") else str(v) if v is not None else None for k, v in s.items()}
                            for s in signals
                        ],
